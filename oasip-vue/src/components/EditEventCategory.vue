...
<template>
<div>  
  <span v-show="repeatablename" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded absolute mx-20 top-44 ">Category Name must be unique Unique <button @click="repeatablename = false">x</button></span>
  <p class="text-3xl font-bold mb-3 ml-4 mt-6	">CATEGORY NAME</p>
  <p class="ml-4 "> <span class="font-bold"> CATEGORY NAME : </span>  <input type="text"  maxlength="100" class="border-2 border-black text-black ml-1 mt-16 bg-zinc-300 disabled:opacity-50 disabled:hover:cursor-not-allowed	" v-model="newEventCategory.eventCategoryName" :onchange="uniqueName">
  ...
  <button :disabled="isDisabled"  @click="isDisabled === true ? '' : $emit('updateEventCategory', {id: newEventCategory.id, eventCategoryName: newEventCategory.eventCategoryName.trimStart().trimEnd() , eventCategoryDescription: newEventCategory.eventCategoryDescription, eventDuration: newEventCategory.eventDuration})" 
   class="text-white bg-black mr-4 border border-solid hover:bg-[#855B52]  active:bg-cyan-600 font-bold uppercase text-sm py-3 rounded outline-none focus:outline-none ease-linear transition-all duration-150 active show px-3disabled:opacity-50 disabled:hover:cursor-not-allowed ml-4 mb-4"> Save</button>
</div>
</template>
